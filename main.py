...
# Load the pre-trained model (SSD MobileNet V3)
...
def update():
    ...
    img_resized = cv2.resize(img, (700, 500))

    # Detect objects in the image if detection is running
    if is_detection_running:
        classIds, confs, bbox = net.detect(img_resized, confThreshold=thres)
        ...
        if len(classIds) != 0:
            for classId, confidence, box in zip(classIds.flatten(), confs.flatten(), bbox):
                ...
                if 0 <= classId < len(classNames):
                    # Draw bounding box
                    cv2.rectangle(img_resized, box, color=(0, 255, 0), thickness=2)
                    cv2.putText(img_resized, classNames[classId].upper(), (box[0] + 10, box[1] + 30),
                                cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(img_resized, str(round(confidence * 100, 2)), (box[0] + 200, box[1] + 30),
                                cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
